...
function App() {
  ...
  const showComment = async (postId) => {
    ...
    setState((prev) => ({ ...prev, [postId]: true }));
  }
  console.log(state)

  const hide = (postId) =>{
    setState((prev) => ({ ...prev, [postId]: false }));
  }
  useEffect(()=>{
    allData()
  },[])
  return (
    <div className="App">
      <div className="px- py-5 ">
        <h1 className='font-bold text-3xl uppercase text-orange-500'>All Posts</h1>
      </div>
      {post.map((res)=> 
        <div className='antialiased border border-slate-400 mx-2 my-3 px-4 py-3 rounded-md'>
          ...
          <button onClick={()=>hide(res.id)} className='bg-slate-500 hover:bg-slate-400 text-white font-semibold px-4 py-1 rounded-full'>
